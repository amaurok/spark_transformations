...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    numberRatings = 0
    sumRatings = 0
    for i in IDandRatingsTuple[1]:
        numberRatings += 1
        sumRatings += i
    
    return (IDandRatingsTuple[0], (numberRatings,float(sumRatings)/float(numberRatings)))
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda (UserID, MovieID, Rating): (MovieID, Rating)).groupByKey())
...
movieNameWithAvgRatingsRDD = (moviesRDD.join(movieIDsWithAvgRatingsRDD).map(lambda (x, y): (y[1][1], y[0], y[1][0]))
                              )
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD.join(actualReformattedRDD).map(lambda (x, y): math.pow((y[0] - y[1]),2)))
...
trainingAvgRating = float(trainingRDD.map(lambda (UserID, MovieID, Rating): Rating ).reduce(lambda a, b: a + b))/float(trainingRDD.count())
...
myUnratedMoviesRDD = moviesRDD.filter(lambda x: x[0] not in myRatedMovies[1]).map(lambda x: (0, x[0]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD.join(moviesRDD).map(lambda (x, y): (y[0][0], y[1], y[0][0]))
                       )
