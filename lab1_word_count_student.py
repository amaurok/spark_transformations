...
# TODO: Replace <FILL IN> with appropriate code
pluralLengths = (pluralRDD.map(lambda word: len(word))
                 .collect())
print pluralLengths
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda (Column, values): (Column, sum(values)))
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsCollected = (wordsRDD.map(lambda word: (word,1)).reduceByKey(lambda a, b: a+b)
                       .collect())
...
totalCount = (wordCounts
              .map(lambda (Column, values): (values))
              .reduce(lambda a, b: a + b)
              )
average = totalCount / float(len(wordCountsCollected))
print totalCount
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakespeareWordsRDD).takeOrdered(15, key = lambda (Column, values): -1*values)
