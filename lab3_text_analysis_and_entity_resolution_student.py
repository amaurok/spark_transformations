...
def simpleTokenize(string):
    ...
    """
    return filter(None, re.split('\W+',string.lower()))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (x,y): len(y)).reduce(lambda x, y: x + y)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(3,lambda (x,y): -1 * len(y))
...
def tf(tokens):
    ...
    for k, v in new_dict.items():
        new_dict[k] = float(v)/float(len(tokens))
...
def idfs(corpus):
    ...
    """
    N = corpusRDD.count()
    uniqueTokens = corpus.flatMap(lambda (x, y): set(y))
    tokenCountPairTuple = uniqueTokens.map(lambda y: (y,1))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a + b)
    return (tokenSumPairTuple.map(lambda (x, y) : (x, float(N)/float(y))))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {k: float(tfs[k]) * float(idfs[k]) for k in tfs}
    return tfIdfDict
...
trueDupsRDD = (sims.join(goldStandard)).map(lambda (x, y): y[0])
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.reduce(lambda a, b: a + b)/float(trueDupsCount)

nonDupsRDD = (sims.subtractByKey(goldStandard))
avgSimNon = nonDupsRDD.map(lambda (x, y): y).reduce(lambda a, b: a + b)/float(nonDupsRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda tokens: (tokens[0], norm(tokens[1]))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda tokens: (tokens[0], norm(tokens[1]))).collectAsMap()
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum([amazonWeightsBroadcast.value[amazonRec][t]
                 * googleWeightsBroadcast.value[googleRec][t] for t in tokens])
             
    amazonNorm = amazonNormsBroadcast.value.get(amazonRec)
    googleNorm = googleNormsBroadcast.value.get(googleRec)

    value = float(s)/float(amazonNorm * googleNorm)
